...
function findLongestWord(sentence) {
  ...
  const longestLength = arr.map((el) => el.length).sort((a, b) => b - a)[0];
  return arr.filter((el) => el.length === longestLength)[0];
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const resultArr = arr.map((el) => el.split('').reverse().join(''));
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) =>
      alphabet.includes(el) ? encodedAlphabet[alphabet.indexOf(el)] : el
    )
    .join('');
}
